...
/*
	add:2015-08-21
	func:get the correct ip address
*/
void 
_eXosip_define_get_ip_0(char *locip, int size, osip_message_t *message)
{
	if (!locip || (size <= 0) || !message)
	{
		return;
	}
	int type = 0;
	...
	char wlan0Mask[32] = {0};
	char compareIp[32] = {0};
	P_getLocalIpAddress_F getlocalip_func = get_get_localip_func();

	getlocalip_func(&type, eth0Ip, wlan0Ip, wlan0Mask);
	if (0 == type)
	{
		snprintf(locip, size, "%s", eth0Ip);
		return;
	}
	
	if (message->from && message->from->url && message->from->url->host)
	{
		snprintf(compareIp, sizeof(compareIp), "%s", message->from->url->host);
	}	
	else if (message->to && message->to->url && message->to->url->host)
	{
		snprintf(compareIp, sizeof(compareIp), "%s", message->to->url->host);
	}

	if ((inet_addr(wlan0Ip) & inet_addr(wlan0Mask)) == (inet_addr(compareIp) & inet_addr(wlan0Mask)))
	{
		snprintf(locip, size, "%s", wlan0Ip);
	}
	else
	{
		snprintf(locip, size, "%s", eth0Ip);
										...
										osip_message_t * request)
{
	...
	#if 1
	//add.2014-10-18, mod.2015-08-21
	/*if ((request->from) || (request->to))
	{
		_eXosip_define_get_ip_0(locip, sizeof(locip), request);
	}
	else
	{
    ...
    _eXosip_define_get_ip_0(locip, sizeof(locip), request); //mod for coverity bug (CID:2249392)by ldf 1.5.0    
	#else
	eXosip_guess_ip_for_via(eXosip.eXtl->proto_family, locip, 49);
	#endif
